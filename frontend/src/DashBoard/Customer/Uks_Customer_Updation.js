...
import Uks_Customer_AddressForm from "./Updation/Uks_Customer_Address";
import Uks_Customer_Previous_Loan_Details from "./Updation/Uks_Customer_Previous_Loan";
import Uks_Customer_Salaried_Person from "./Updation/Uks_Customer_Salaried_Person";
import Uks_Customer_LoanProcessing from "./Updation/Uks_Customer_LoanProcessing";

function Uks_Customer_Updation() {
    ...
    const handleProfileDownload = () => {
        if (customerId) {
            navigate(`/customer/profile/download`, { state: { customerId, uksId } });
        }
    };
        ...
        <>
            <Container fluid className={`Customer-basic-view-container ${isSidebarExpanded ? 'sidebar-expanded' : ''}`}>
                <div style={{ paddingBottom: '18px' }}>
                    <PathnameUrlPath location={location} homepage={() => navigate('/uks/dashboard', { state: { uksId } })} />
                </div>
                <Row className="Section-1-Row">
                    <Col className="New-Customer-container-second basic-view-col">
                        ...
                        <hr />
                        <Uks_Customer_AddressForm />
                        <hr />
                        <Uks_Customer_Previous_Loan_Details />
                        <hr />
                        {/* Conditionally render Customer_Salaried_Person based on customerType */}
                        {formData.customerType === 'Salaried Person' && <Uks_Customer_Salaried_Person />}
                        < Uks_Customer_LoanProcessing refetchData={refetchData} />
                    </Col>
                </Row>
            </Container>
        </>
...
export default Uks_Customer_Updation;
