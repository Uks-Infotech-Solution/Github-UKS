...
const Loan_Level = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
<<<<<<< HEAD
      const response = await axios.post('https://uksinfotechsolution.in:8000/api/loan-levels', { loanLevel: documentType });
=======
      const response = await axios.post('https://localhost:8000/api/loan-levels', { loanLevel: documentType });
  ...
  return (
    <Container fluid className={`apply-loan-view-container ${isSidebarExpanded ? 'sidebar-expanded' : ''}`}>
      <div style={{ maxWidth: '600px', margin: 'auto', paddingTop: '20px' }}>
        <h2 style={{ marginBottom: '20px', textAlign: 'center', color: '#333' }}>Add Loan Level</h2>
        <form onSubmit={handleSubmit} style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}>
          <div style={{ marginBottom: '15px' }}>
            <label htmlFor="documentType" style={{ display: 'block', marginBottom: '5px', fontSize: '16px' }}>Loan Level:</label>
            <input
              type="text"
              id="documentType"
              value={documentType}
